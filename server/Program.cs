...
// Явно привязываем backend к тому адресу, куда ходит Vite proxy
builder.WebHost.UseUrls("http://127.0.0.1:5165");
...
{
    int total = rng.Next(10, 100);
    int needsRepair = rng.Next(1, Math.Max(2, total / 5));
    int working = total - needsRepair;
    int lowSupply = rng.Next(1, Math.Max(2, working / 2));
...
app.MapGet("/sales/index-by-historic-avg", () =>
    machines
        .Select(x => new SalesIndex(x.id, x.type, rng.Next(1, 100)))
...
app.MapGet("/machines/product-fill", () =>
{
    ...
    var vmItemFills = machines
        .Select(x =>
        {
            int itemCount = rng.Next(1, max);
            return new VendingMachineItemFill(itemCount, itemCount * 100 / max);
        })
        .OrderBy(x => x.fillPercentage)
        .ToArray();

    return new VendingMachinesItemFillOverview(
        vmItemFills.Sum(x => x.itemCount),
        vmItemFills
    );
...
app.MapGet("/machines/money-fill", () =>
    machines
        .Select(x => new VendingMachineMoneyStatus(
            x.id,
            x.type,
            rng.Next(1, 100),
            rng.Next(1, 100)
        ))
        .OrderByDescending(x => x.coinFillPercentage + x.banknotesFillPercentage)
        .ToArray()
).WithName("GetVendingMachinesMoneyFill");

app.MapGet("/sales/by-vending-machine", () =>
{
    int totalSales = rng.Next(1000, 10000);
    int max = Math.Max(101, totalSales / 5);

    var topVmSales = machines
        .Select(x =>
        {
            int sold = rng.Next(100, max);
            return new VendingMachineTotalSales(sold, sold * 100 / totalSales);
        })
        .OrderByDescending(x => x.totalSales)
        .ToArray();

    return new VendingMachinesTotalSalesOverview(
        totalSales,
        topVmSales.Sum(x => x.totalSales),
        topVmSales
    );
...
{
    int totalSold = rng.Next(10_000, 100_000);
    int max = Math.Max(1001, totalSold / 8);

    var topProductSales = productIds
        .Select(x =>
        {
            int sold = rng.Next(1000, max);
            return new ProductTotalSales(
                x,
                rng.Next(1, 5),
                sold,
                sold * 100 / totalSold
        ...
        .OrderByDescending(x => x.soldTotal)
        .ToArray();

    return new ProductsTotalSalesOverview(
        totalSold,
        topProductSales.Sum(x => x.soldTotal),
        topProductSales.DistinctBy(x => x.category).Count(),
        topProductSales
    );
...
app.MapGet("/sales/peak-sale-count-per-day", () =>
    Enumerable
        .Range(1, 31)
        .Select(x => new PeakSaleTimeAtDay(x, GetRandomTimeOfDay(rng)))
        .ToArray()
).WithName("GetPeakSaleCountPerDay");

app.Run();

static TimeSpan GetRandomTimeOfDay(Random rng)
{
